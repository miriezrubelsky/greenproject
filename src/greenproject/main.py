...
def load_data():
    ...
    return offsets, parameters, tif_files

def process_image_file(tif_file, offsets, parameters):
    """Process each TIFF file."""
    offset_per_pic = offsets.get(tif_file)
    file_parameters = parameters.get(tif_file)

    if file_parameters is None or offset_per_pic is None:
        appLogger.getLogger().debug(f"Skipping {tif_file} due to missing parameters or offsets.")
    ...
    for root, _, files in os.walk(LOCAL_DIR):
        for png_image in tqdm.tqdm([f for f in files if f.endswith('.png')], desc=f"Processing images in {root}"):
            ...
            polygons = processor.process_image(png_image_path, file_parameters['transform'])
            adjusted_polygons.extend(polygons)
    
    # Post-process and save shapefile
    postprocessor = PostImageProcessor(tif_file, adjusted_polygons, file_parameters['crs'])
...
def main():
    ...
    offsets, parameters, tif_files = load_data()
    total_data_size =0
    for tif_file in tif_files:
        data_size = len(tif_file)
        start_time = datetime.now()
        output_path = process_image_file(tif_file, offsets, parameters)
